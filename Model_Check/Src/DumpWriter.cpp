									   ...
									   const API_Element*	wallelem)
{
	...
	if ((elemHead->typeID == API_WindowID || elemHead->typeID == API_DoorID) && wallelem != nullptr)
								 ...
								 Int32 					dim2)
{
	...
	switch (typeID) {
		case APIParT_FillPat:	head.Append (" \"");
								DumpUtils::GetAttrName (nullptr, API_FilltypeID, (short)value, &name);
								head.Append (name);
								head.Append ("\"");
		...
		case APIParT_Mater:		head.Append (" \"");
								DumpUtils::GetAttrName (nullptr, API_MaterialID, (short)value, &name);
								head.Append (name);
								head.Append ("\"");
								WriteStr (head, WithNewLine);
								break;
		...
		case APIParT_BuildingMaterial:
								head.Append (" \"");
								DumpUtils::GetAttrName (nullptr, API_BuildingMaterialID, (short)value, &name);
								head.Append (name);
								head.Append ("\"");
								WriteStr (head, WithNewLine);
								break;
		...
		case APIParT_Profile:	head.Append (" \"");
								DumpUtils::GetAttrName (nullptr, API_ProfileID, (short)value, &name);
								head.Append (name);
								head.Append ("\"");
								WriteStr (head, WithNewLine);
								break;
		...
		case APIParT_LineTyp:	head.Append (" \"");
								DumpUtils::GetAttrName (nullptr, API_LinetypeID, (short)value, &name);
								head.Append (name);
								head.Append ("\"");
								WriteStr (head, WithNewLine);
								break;
...
GSErrCode DumpWriter::WrProperties (const API_Guid& guid)
{
	...
	API_ComponentRefType**	comphandle = nullptr;
	if (err == NoError) {
...
GSErrCode DumpWriter::Wr_Morph (const API_Element* elem)
{
	...
	for (ULong j = 0; j < memo.morphBody->GetPolygonCount (); ++j) {
		...
		if ((BMpGetSize (reinterpret_cast<GSPtr> (memo.morphMaterialMapTable)) / sizeof (API_OverriddenAttribute)) >= j) {
			WriteStr ("Material ");
			WriteInt (memo.morphMaterialMapTable[j].attributeIndex);
		}
