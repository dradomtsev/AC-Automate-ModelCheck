...
//
// Module:			API Development Kit 24; Mac/Win
// Namespace:		-none-
...
static GSErrCode	Do_SaveAs_Wall (void)
{
	...
	for (GS::Array<API_Guid>::ConstIterator it = wallList.Enumerate (); it != nullptr && err == NoError; ++it) {
		...
		if (err == NoError) {
			...
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.wall.refMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.wall.oppMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.wall.sidMat.attributeIndex, &name);
			writer->WriteName (name, DumpWriter::WithNewLine);
...
static GSErrCode	Do_SaveAs_Column (void)
{
	...
	for (GS::Array<API_Guid>::ConstIterator it = columnList.Enumerate (); it != nullptr && err == NoError; ++it) {
		...
		if (err == NoError) {
			...
			DumpUtils::GetAttrName (nullptr, API_MaterialID, memo.columnSegments[0].extrusionSurfaceMaterial.attributeIndex, &buffer);
	...
	for (GS::Array<API_Guid>::ConstIterator it = slabList.Enumerate (); it != nullptr && err == NoError; ++it) {
		...
		if (err == NoError) {
			...
			if (err == NoError) {
				...
				DumpUtils::GetAttrName (nullptr, API_MaterialID, element.slab.topMat.attributeIndex, &name);
				writer->WriteName (name);
				DumpUtils::GetAttrName (nullptr, API_MaterialID, element.slab.botMat.attributeIndex, &name);
				writer->WriteName (name);
				DumpUtils::GetAttrName (nullptr, API_MaterialID, element.slab.sideMat.attributeIndex, &name);
				writer->WriteName (name, DumpWriter::WithNewLine);
...
static GSErrCode Do_SaveAs_Roof (void)
{
	...
	for (GS::Array<API_Guid>::ConstIterator it = roofList.Enumerate (); it != nullptr && err == NoError; ++it) {
		...
		if (err == NoError) {
			...
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.roof.shellBase.topMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.roof.shellBase.botMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.roof.shellBase.sidMat.attributeIndex, &name);
			writer->WriteName (name, DumpWriter::WithNewLine);
...
static GSErrCode Do_SaveAs_Shell (void)
{
	...
	for (GS::Array<API_Guid>::ConstIterator it = shellList.Enumerate (); it != nullptr && err == NoError; ++it) {
		...
		if (err == NoError) {
			...
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.shell.shellBase.topMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.shell.shellBase.botMat.attributeIndex, &name);
			writer->WriteName (name);
			DumpUtils::GetAttrName (nullptr, API_MaterialID, element.shell.shellBase.sidMat.attributeIndex, &name);
			writer->WriteName (name, DumpWriter::WithNewLine);
...
static void	DumpElem (const API_Guid& guid, DifferenceTypeID diffType, GS::UniString& reportStr)
{
	...
	if (diffType != DELETED_ELEM) {
		...
		if (ACAPI_Element_GetHeader (&elemHead) == NoError && elemNames.ContainsKey (elemHead.typeID))
			reportStr += "Type: " + elemNames[elemHead.typeID] + "\n";
	}
...
static void	Do_StoreProjectForCompare ()
{
	...
	GSErrCode err = ACAPI_ElemDifferenceGenerator_GetState (APIDiff_ModificationStampBased, &diffGeneratorState);
	if (err != NoError)
		ErrorBeep ("APIDiff_ModificationStampBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
}
...
static void	Do_CompareToStoredProject ()
{
	...
	GSErrCode err = ACAPI_ElemDifferenceGenerator_GenerateDifference (APIDiff_ModificationStampBased, &diffGeneratorState, nullptr, &elemDifference);
	if (err != NoError) {
		ErrorBeep ("APIDiff_ModificationStampBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
	} else {
...
static void	Do_StoreModelForCompare ()
{
	...
	GSErrCode err = ACAPI_ElemDifferenceGenerator_GetState (APIDiff_3DModelBased, &diffGeneratorState);
	if (err != NoError)
		ErrorBeep ("APIDiff_3DModelBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
}
...
static void	Do_CompareToStoredModel ()
{
	...
	GSErrCode err = ACAPI_ElemDifferenceGenerator_GenerateDifference (APIDiff_3DModelBased, &diffGeneratorState1, &diffGeneratorState2, &elemDifference);
	if (err != NoError) {
		ErrorBeep ("APIDiff_3DModelBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
	} else {
...
static void Do_StoreContextForCompare ()
{
	...
	GSErrCode err = NoError;
	{
		...
		err = ACAPI_Navigator (APINavigator_SearchNavigatorItemID, &parentNavigatorItem, nullptr, &navigatorItemsArray);
	...
	err = ACAPI_ElemDifferenceGenerator_GetState (APIDiff_ContextBased, &diffGeneratorState);
	if (err != NoError)
		ErrorBeep ("APIDiff_ContextBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
}
...
static void	Do_CompareToStoredContext ()
{
	...
	err = ACAPI_ElemDifferenceGenerator_GenerateDifference (APIDiff_ContextBased, &diffGeneratorState1, &diffGeneratorState2, &elemDifference);
	if (err != NoError) {
		ErrorBeep ("APIDiff_ContextBased ACAPI_ElemDifferenceGenerator_GetState failed", err);
	} else {
...
GSErrCode __ACENV_CALL	RegisterInterface (void)
{
	...
	// Register menus
	err = ACAPI_Register_Menu (IDR_PlanDumpMenu, 0, MenuCode_UserDef, MenuFlag_SeparatorBefore); //or MenuFlag_Default
...
{
	ACAPI_Install_MenuHandler (IDR_PlanDumpMenu, MenuCommandProc);
